...
/**
 * Account lifecycle: registration, login, linking, and role profiles.
 *
 * An `Account` is the person. `User` (creator) and `Worker` are profiles that
 * hang off it and are created lazily, the first time the account does something
 * in that role — so signing up never forces a "are you a creator or a worker?"
 * question, and a person who does both has one login and one audit trail.
 */
...
export interface AuthResult extends SessionToken {
  account: ReturnType<typeof toPublicAccount>;
  /** True only on the request that created the account, so the UI can onboard. */
  isNewAccount?: boolean;
}

function issueToken(accountId: number): SessionToken {
  return {
...
function assertUsable(account: Account): void {
...
export function toPublicAccount(
  account: Account & { creatorProfile?: unknown; workerProfile?: unknown },
) {
  return {
    id: account.id,
    email: account.email,
...
export async function registerWithEmail(input: {
  ...
  if (weakness) throw badRequest(weakness, "WEAK_PASSWORD");
  ...
  await issueEmailVerification(account.id, email, input.context).catch((error) =>
    logger.error("Verification email failed at registration", {
 ...
 */
export function buildWalletChallenge(nonce: string, purpose: "signin" | "link"): string {
...
function verifyWalletSignature(walletAddress: string, message: string, signature: unknown): void {
  ...
  if (!verified) throw unauthorized("Wallet signature could not be verified", "INVALID_SIGNATURE");
}
...
export async function authenticateWithWallet(input: {
  ...
  verifyWalletSignature(
    input.walletAddress,
    buildWalletChallenge(input.nonce, "signin"),
    input.signature,
  );
...
export async function linkWallet(
  accountId: number,
  input: { walletAddress: string; signature: unknown; nonce: string; context?: AuditContext },
) {
  verifyWalletSignature(
    input.walletAddress,
    buildWalletChallenge(input.nonce, "link"),
    input.signature,
  );
  ...
  if (account.walletAddress) {
    throw conflict(
      "This account already has a wallet linked. Unlink it first.",
  ...
  if (taken) {
    throw conflict(
      "That wallet is already linked to another DojoPay account.",
      "WALLET_IN_USE",
    );
  }
 ...
 */
export async function unlinkWallet(accountId: number, context?: AuditContext) {
  const account = await loadAccount(accountId);
  ...
  if (!account.passwordHash && !account.googleId) {
    throw badRequest(
      "Add an email and password or connect Google before removing your wallet, " +
        "or you will not be able to sign in again.",
      "WALLET_IS_ONLY_CREDENTIAL",
    );
  }

  if (account.workerProfile && account.workerProfile.pending_amount > 0n) {
    throw badRequest(
      "Withdraw your pending earnings before unlinking your wallet.",
      "PENDING_BALANCE",
    );
  }
...
export async function linkEmail(
  accountId: number,
  input: { email: string; password: string; context?: AuditContext },
) {
  const email = normaliseEmail(input.email);
  const account = await loadAccount(accountId);
  ...
  await issueEmailVerification(accountId, email, input.context).catch((error) =>
    logger.error("Verification email failed on link", {
      accountId,
      error: error instanceof Error ? error.message : String(error),
    }),
  );
...
export async function resendVerification(accountId: number, context?: AuditContext) {
  const account = await loadAccount(accountId);
  if (!account.email) throw badRequest("No email on this account", "NO_EMAIL");
  if (account.emailVerified) throw badRequest("Email is already verified", "ALREADY_VERIFIED");

  await issueEmailVerification(accountId, account.email, context);
  return { message: "Verification email sent" };
}

export async function verifyEmail(token: string, context?: AuditContext) {
  ...
  if (record.usedAt) throw badRequest("That link has already been used", "TOKEN_USED");
...
export async function requestPasswordReset(email: string, context?: AuditContext) {
  ...
  if (account?.passwordHash || account?.email) {
    ...
    await sendPasswordResetEmail(normalised, token).catch((error) =>
...
export async function changePassword(
  accountId: number,
  input: { currentPassword: string; newPassword: string; context?: AuditContext },
) {
  const account = await loadAccount(accountId);
...
export async function updateProfile(
  accountId: number,
  input: { displayName?: string; context?: AuditContext },
) {
  const updated = await prismaClient.account.update({
    where: { id: accountId },
 ...
 */
export async function ensureCreatorProfile(accountId: number, context?: AuditContext) {
...
export async function ensureWorkerProfile(accountId: number, context?: AuditContext) {
