...
function verifySignature(publicKey: string, signature: unknown, message: string): void {
  ...
  if (!verified) throw unauthorized("Incorrect signature", "INVALID_SIGNATURE");
}

export async function signInCreator(publicKey: string, signature: unknown) {
...
export async function signInWorker(publicKey: string, signature: unknown, referredBy?: string) {
