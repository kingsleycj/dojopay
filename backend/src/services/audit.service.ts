...
export const AuditAction = {
  ...
  VAULT_WITHDRAWAL_REQUESTED: "VAULT_WITHDRAWAL_REQUESTED",
  VAULT_WITHDRAWAL_SUCCEEDED: "VAULT_WITHDRAWAL_SUCCEEDED",
  VAULT_WITHDRAWAL_FAILED: "VAULT_WITHDRAWAL_FAILED",
  VAULT_TASK_FUNDED: "VAULT_TASK_FUNDED",
  VAULT_TASK_REFUNDED: "VAULT_TASK_REFUNDED",
...
const SEVERITY_BY_ACTION: Partial<Record<AuditActionType, AuditSeverity>> = {
  ...
  VAULT_DEPOSIT: AuditSeverity.NOTICE,
  VAULT_WITHDRAWAL_REQUESTED: AuditSeverity.NOTICE,
  VAULT_WITHDRAWAL_SUCCEEDED: AuditSeverity.NOTICE,
  VAULT_WITHDRAWAL_FAILED: AuditSeverity.WARNING,
  TASK_CANCELLED: AuditSeverity.NOTICE,
...
export function auditAccount(
  ...
  action: AuditActionType,
  options: Omit<RecordAuditInput, "action" | "actorType" | "actorAccountId"> = {},
): Promise<void> {
  return recordAudit({
    ...options,
    action,
...
export function auditAdmin(
  adminId: number,
  action: AuditActionType,
  options: Omit<RecordAuditInput, "action" | "actorType" | "actorAdminId"> = {},
): Promise<void> {
  return recordAudit({
    ...options,
    action,
...
export function auditSystem(
  action: AuditActionType,
  options: Omit<RecordAuditInput, "action" | "actorType"> = {},
): Promise<void> {
...
export async function accountTimeline(accountId: number, limit = 100) {
