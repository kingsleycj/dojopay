    ...
    try {
        ...
        const allEarnings = [
            ...submissions.map((s) => ({
                ...
                status: 'pending', // All submissions are pending until paid out
                taskId: s.task_id,
...
router.post("/payout", workerAuthMiddleware, async (req, res) => {
    ...
    const isSignatureValid = nacl.sign.detached.verify(withdrawalMessage, signatureBytes, new PublicKey(worker.address).toBytes());
    if (!isSignatureValid) {
        return res.status(401).json({
