...
import { AccountStatus, ActorType, PayoutStatus, TaskStatus, } from "@prisma/client";
...
import { AuditAction, auditAdmin, recordAudit } from "./audit.service.js";
...
function issueChallengeToken(adminId, purpose) {
...
function verifyChallengeToken(token, purpose) {
    try {
        const decoded = jwt.verify(token, config.auth.adminJwtSecret);
...
export async function createAdmin(input) {
    ...
    if (existing)
        throw conflict("An admin with that email already exists", "ADMIN_EXISTS");
...
export async function getOverview() {
    ...
    const [totalAccounts, newAccountsToday, newAccountsWeek, suspendedAccounts, walletLinked, totalTasks, openTasks, totalSubmissions, submissionsToday, payoutAgg, failedPayouts, pendingAgg, recentSignups, criticalEvents,] = await Promise.all([
    ...
    return {
        accounts: {
            ...
            walletLinkRate: totalAccounts > 0 ? ((walletLinked / totalAccounts) * 100).toFixed(1) : "0",
        },
...
export async function getAccountDetail(accountId, viewedByAdminId) {
...
export async function moderateAccount(input) {
    ...
    if (!account)
        throw notFound("Account not found", "ACCOUNT_NOT_FOUND");
    const status = input.action === "REACTIVATE"
    ...
    const action = input.action === "REACTIVATE"
    ...
    if (account.email && input.action !== "REACTIVATE") {
        await sendAccountSuspendedEmail(account.email, input.reason).catch((error) => logger.error("Suspension email failed", {
