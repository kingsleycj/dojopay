...
export const DashboardView = () => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          console.log(
            "Dashboard authentication failed, clearing token and redirecting",
          );
  ...
  useEffect(() => {
    ...
    const maxPage = Math.max(
      0,
      Math.ceil(data.recentActivity.length / itemsPerPage) - 1,
    );
    ...
    switch (chartView) {
      case "daily":
        return data.dailyStats.slice(-7).map((stat) => ({
          label: new Date(stat.date).toLocaleDateString("en", {
            weekday: "short",
          }),
          fullLabel: new Date(stat.date).toLocaleDateString("en", {
            weekday: "long",
            month: "short",
            day: "numeric",
            year: "numeric",
  ...
  const totalSpentUsd =
    solPriceUsd && Number.isFinite(totalSpentSol)
      ? totalSpentSol * solPriceUsd
      : null;
      ...
      <div className="flex flex-col gap-3 mb-4 sm:mb-6">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-600 mt-1">
            Overview of tasks, submissions, and spend.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-4 lg:p-5">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mt-1 truncate">
                {data.overview.totalSubmissions}
              </p>
            </div>
          ...
          <div className="mt-2 sm:mt-4 text-xs sm:text-sm text-gray-600">
            ...
            <span className="ml-1 sm:ml-2 font-semibold text-gray-900">
              {data.overview.averageSubmissionsPerTask}
            </span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-4 lg:p-5">
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mt-1 truncate">
                {totalSpentSolStr} SOL
              </p>
              <p className="text-xs sm:text-sm text-green-600 mt-1">
                {totalSpentUsd === null
                  ? "—"
                  : totalSpentUsd.toLocaleString("en-US", {
                      style: "currency",
                      currency: "USD",
                    })}
              </p>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-4 lg:p-5">
          ...
          <div className="mt-2 sm:mt-4 h-2 rounded-full bg-gray-100 overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, Math.max(0, data.completionTrend?.[data.completionTrend.length - 1]?.completionRate ?? 0))}%`,
              }}
            />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-600">
              Tasks created vs submissions received.
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 p-3 sm:p-4 lg:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3 sm:mb-4">
          ...
          <div className="text-xs text-gray-500">
            Last {Math.min(itemsPerPage, filteredRecentActivity.length)} shown
          </div>
        </div>
        <div className="space-y-2 sm:space-y-3">
          {filteredRecentActivity.length === 0 ? (
            <p className="text-gray-500 text-center py-3 sm:py-4">
              No recent activity
            </p>
          ) : (
            <>
              {paginatedActivity.map((activity) => (
                <div
                  key={activity.id}
                  className="flex flex-col gap-2 sm:gap-0 p-3 sm:p-4 bg-gray-50 rounded-xl border border-gray-200/60"
                >
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-900 text-sm sm:text-base line-clamp-2 pr-16 sm:pr-0">
                      {activity.title}
                    </h4>
                  ...
                  <div className="flex flex-row sm:flex-col items-end justify-between gap-2 sm:gap-1">
                    <span
                      className={`inline-block px-2 py-1 text-xs rounded-full flex-shrink-0 ${
              ...
              {totalPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-4">
                  <button
                    onClick={() =>
                      setActivityPage(Math.max(0, activityPage - 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setActivityPage(
                        Math.min(totalPages - 1, activityPage + 1),
                      )
                    }
