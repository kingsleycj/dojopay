                ...
                onSelect={async () => {
                  ...
                  try {
                    ...
                    if (
                      ...
                      console.log(
                        "Submission authentication failed, clearing token and redirecting",
                      );
                      localStorage.removeItem("workerToken");
                      window.location.href = "/";
                    } else if (
                      ...
                      e.response.data.message
                        .toLowerCase()
                        .includes("task is no longer available")
                    ) {
                      showToast(
                        "That task was just taken. Loading a new one...",
                        "info",
                      );
                      await fetchTasks();
                    } else {
          ...
          {submitting && (
            <div className="flex justify-center">
              <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-lg px-4 sm:px-6 py-3 sm:py-4 shadow-sm">
                ...
                <span className="text-xs sm:text-sm sm:text-base text-gray-700 font-medium">
...
function Option({
  ...
  if (!imageUrl || hasError) {
    return (
      <div className="p-2 sm:p-4 border rounded-lg m-1 sm:m-2 bg-white shadow-sm">
  ...
  return (
    <div className="p-2 sm:p-4 border rounded-lg m-1 sm:m-2 bg-white shadow-sm">
      <img
        className={
          "p-1 sm:p-2 w-full max-w-xs sm:max-w-sm rounded-lg object-cover cursor-pointer hover:opacity-80 transition-opacity"
        }
        src={imageUrl}
