...
export const TasksView = () => {
    ...
    const fetchTasks = async () => {
      try {
        ...
        if (error.response?.status === 401 || error.response?.status === 403) {
          console.log(
            "Tasks authentication failed, clearing token and redirecting",
          );
  ...
  return (
    <div className="max-w-6xl mx-auto p-3 sm:p-4 lg:p-6">
      <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-800 mb-3 sm:mb-4 lg:mb-6">
        Your Tasks
      </h1>
      ...
      {tasks.length === 0 ? (
        <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
          <div className="text-center text-gray-500">
            <div className="mb-3 sm:mb-4">
              <svg
                className="w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 mx-auto text-gray-400"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-4">
              You haven't created any tasks yet. Go to "Create Task" to get
              started!
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
          {tasks.map((task) => (
            <div
              key={task.id}
              className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow relative"
            >
              <div className="p-3 sm:p-4 lg:p-6 pt-4 sm:pt-5 flex flex-col h-full">
                ...
                <div className="mb-3 min-h-[2.5rem] sm:min-h-[3rem]">
                  <h3
                    className="text-sm sm:text-base lg:text-lg font-semibold text-gray-800 line-clamp-2 leading-tight"
                    title={task.title}
                  >
                ...
                <div className="flex gap-2 mb-3 sm:mb-4 min-h-[3rem] sm:min-h-[3rem]">
                  {task.options.slice(0, 3).map((option, index) => (
                    <div
                      key={option.id}
                      className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden border border-gray-200 flex-shrink-0"
                    >
                      <img
                        src={option.imageUrl}
                        alt={`Option ${index + 1}`}
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-auto">
                  ...
                  {task.status === "pending" && (
                    <button
                      onClick={() =>
                        router.push(`/creator/task/${task.id}/edit`)
                      }
