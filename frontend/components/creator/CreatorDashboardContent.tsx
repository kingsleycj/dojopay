...
export const CreatorDashboardContent = ({
  onTaskCreate,
  onViewTasks,
  onViewEarnings,
  ...
  const analyticsTotalPages = Math.ceil(
    analyticsData.length / analyticsItemsPerPage,
  );
  ...
  const activityTotalPages = Math.ceil(
    data.recentActivity.length / activityItemsPerPage,
  );
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 hover:shadow-md transition-shadow">
              ...
              <p className="text-xs text-gray-500 mt-1">
                {data.overview.averageSubmissionsPerTask} avg per task
              </p>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Latest task updates and submissions
                  </p>
                </div>
                <button
              ...
              {activityTotalPages > 1 && (
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-200">
                  <div className="text-sm text-gray-500">
                    Showing {(activityPage - 1) * activityItemsPerPage + 1} to{" "}
                    {Math.min(
                      activityPage * activityItemsPerPage,
                      data.recentActivity.length,
                    )}{" "}
                    of {data.recentActivity.length} activities
                  </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            {analyticsTotalPages > 1 && (
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-200">
                <div className="text-sm text-gray-500">
                  Showing {(analyticsPage - 1) * analyticsItemsPerPage + 1} to{" "}
                  {Math.min(
                    analyticsPage * analyticsItemsPerPage,
                    analyticsData.length,
                  )}{" "}
                  of {analyticsData.length} items
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() =>
                      setAnalyticsPage((prev) => Math.max(prev - 1, 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setAnalyticsPage((prev) =>
                        Math.min(prev + 1, analyticsTotalPages),
                      )
                    }
